...
class Config:
    ...
    uniform_batching_from_all_views: bool = False  # If True, batch equal number of rays from all views
    ...
    render_train: bool = False  # If True, render training set. (rawnerf_mode NOT supported yet)
    render_ft: bool = False  # If True, render fine-tuned model.
    ...
    vis_decimate: int = 0
    load_state_dict_strict: bool = False  # The 'strict' paramter in load_state_dict.
    ...
    # Mult. on the bilateral grid regularizations.
    ...
    bilgrid_lr_scale: float = 0.1  # Learning rate scale of the bilateral grids.

    # Only used by train_bilagrid.py
    ft_name: str = 'ft_test'
    ft_reload_ckpt: str = '' # The path of fine-tuning checkpoint to reload
    max_ft_steps: int = 2500  # The number of 4D bilateral grid optimization steps.
    ft_tgt_image: Union[str, List[str]] = ''  # The editing target image path(s)
    ft_tgt_pose: Union[str, List[str]] = ''  # The camera pose(s) of the editing target, format: {test|train|path|all}:{idx}
    ft_render_every: int = 500  # Steps between test set renders when fine-tuning
    ft_lr_init: float = 0.01  # The initial learning rate for fine-tuning.
    ft_lr_final: float = 0.001  # The final learning rate for fine-tuning.
    # Mult. on the 4D bilateral grid regularizations.
    bilgrid4d_tv_loss_mult: float = 1.
    ...
    save_acc_maps: bool = False  # If True, save acc. maps.
