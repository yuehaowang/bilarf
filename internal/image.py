...
def rgb_to_gray(rgb):
    ...
    :return: (..., 1)
    """
    return rgb @ np.array([[0.299, 0.587, 0.114]]).T

def resize_images(imgs, factor):
    """Resize images"""
    out = [cv2.resize(im, (im.shape[1] // factor, im.shape[0] // factor), interpolation=cv2.INTER_AREA) for im in imgs]
    return np.stack(out)
...
def linear_to_srgb(linear, eps=None, X=np):
    """Assumes `linear` is in [0, 1], see https://en.wikipedia.org/wiki/SRGB."""
    if eps is None:
        eps = X.finfo(linear.dtype).eps
    srgb0 = 323 / 25 * linear
    srgb1 = (211 * X.maximum(eps, linear) ** (5 / 12) - 11) / 200
    return X.where(linear <= 0.0031308, srgb0, srgb1)

linear_to_srgb_np = partial(linear_to_srgb, X=np)
