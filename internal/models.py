...
class Model(nn.Module):
    ...
    bilateral_grid: bool = False  # If True, create per-view bilateral grids.
    bilateral_grid4d: bool = False  # If True, create 4D bilateral grids for editing.
    ...
    def __init__(self, config=None, **kwargs):
        ...
        if self.bilateral_grid:
            self.bil_grids = nn.ModuleList()
            for i in range(self.num_levels):
                self.bil_grids.append(bilateral_grids.BilateralGrid(self.dataset_info['size']))

        if self.bilateral_grid4d:
            self.bil_grids4d = nn.ModuleList()
            for i in range(self.num_levels):
                self.bil_grids4d.append(bilateral_grids.BilateralGridCP4D())
    ...
    def forward(
        ...
        for i_level in range(self.num_levels):
            ...
            # Apply 4D bilateral grid.
            if self.bilateral_grid4d:
                bilgrid4d_results = bilateral_grids.slice4d(self.bil_grids4d[i_level],
                                                          ray_results['coord'],
                                                          ray_results['rgb'])
            ...
            if self.bilateral_grid and not no_bilgrid:
                ...
                bilgrid_results = bilateral_grids.slice(bil_grids, batch['pix_xy'],
                                                        rendering['rgb'], batch['cam_idx'].long())
                rendering['rgb'] = bilgrid_results['rgb']
                rendering['rgb_affine_mats'] = bilgrid_results['rgb_affine_mats']
                rendering['cam_idx'] = batch['cam_idx']
        ...
        if self.disable_rgb:
            ...
            if viewdirs is not None:
                ...
                if self.bottleneck_width > 0:
                    ...
                    x = [bottleneck] + bottleneck_concat_ls
                else:
                    x = []
    ...
    for i_chunk, idx0 in enumerate(idx0s):
        ...
        with accelerator.autocast():
            chunk_renderings, ray_history = model(rand,
                                                  ...
                                                  compute_extras=True,
                                                  zero_glo=True if not config.render_train else config.zero_glo,
                                                  no_bilgrid=disable_bilgrid)
