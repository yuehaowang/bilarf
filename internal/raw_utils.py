...
def postprocess_raw(raw, camtorgb, camtocamwb=None, camwbtorgb=None, exposure=None):
    ...
    # # White balance
    # raw_wb = raw @ camtocamwb.T
    # raw_wb = np.clip(raw_wb, 0, 1.)
    # # Color transform
    # rgb_linear = raw_wb @ camwbtorgb.T
    ...
    srgb = lib_image.linear_to_srgb(rgb_linear_scaled, X=np)
    return srgb
...
def process_exif(exifs):
    ...
    camwb2rgb = np.linalg.inv(rgb2camwb)
    meta['camwb2rgb'] = camwb2rgb
    # Combining color correction with white balance gives the entire transform.
    cam2rgb = camwb2rgb @ cam2camwb
...
def load_raw_dataset(split, data_dir, image_names, exposure_percentile, n_downsample):
    ...
    cam2camwb0 = meta['cam2camwb'][0]
    camwb2rgb0 = meta['camwb2rgb'][0]
    meta['postprocess_fn'] = lambda z, x=exposure: postprocess_raw(z, cam2rgb0, cam2camwb0, camwb2rgb0, x)
