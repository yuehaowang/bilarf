...
def bilateral_grid4dcp_tv_loss(model, config):
    """Computes total variations of 4D bilateral grids' CP factors."""
    total_loss = 0.

    for bilagrid_4d in model.bil_grids4d:
        # Add TV loss to CP factors.
        for i in range(1, bilagrid_4d.num_facs):
            fac = bilagrid_4d.get_parameter(f'fac_{i}')
            total_loss += config.bilgrid4d_tv_loss_mult * total_variation_loss(fac)
...
def create_optimizer(config: configs.Config, model):
    ...
    if model.bilateral_grid:
        param_groups = [
            {'params': model.bil_grids.parameters(), 'name': 'bil_grids'},
            {'params': [p for k, p in model.named_parameters() if not k.startswith('bil_grids')], 'name': 'nerf'}
        ]
    else:
        param_groups = model.parameters()
    optimizer = torch.optim.Adam(param_groups, lr=config.lr_init, **adam_kwargs)

    return optimizer, lr_fn_main


def create_ft_optimizer(config: configs.Config, model):
    """Creates optimizer for fine-tuning."""
    adam_kwargs = {
        'betas': [config.adam_beta1, config.adam_beta2],
        'eps': config.adam_eps,
    }
    ...
    lr_fn_main = lambda step: math.learning_rate_decay(
        step,
        lr_init=config.ft_lr_init,
        lr_final=config.ft_lr_final,
        max_steps=config.max_ft_steps)
    
    if model.bilateral_grid4d:
        param_groups = [
            {'params': model.bil_grids4d.parameters(), 'name': 'bil_grids4d'}
        ]
    else:
        param_groups = model.parameters()
    optimizer = torch.optim.Adam(param_groups, lr=config.lr_init, **adam_kwargs)
