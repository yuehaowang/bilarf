...
def main(unused_argv):
    ...
    dataloader = torch.utils.data.DataLoader(np.arange(len(dataset)),
                                            #  num_workers=8,
                                             ...
                                             # Set to False to enable switching
                                             # low-res. and high-res. data.
    ...
    if config.resume_from_checkpoint:
        init_step = checkpoints.restore_checkpoint(config.checkpoint_dir, accelerator, logger, strict=True)
    else:
        init_step = 0
    ...
    with logging_redirect_tqdm():
        ...
        for step in tbar:
            ...
            for param_group in optimizer.param_groups:
                ...
                if 'name' in param_group and param_group['name'].startswith('bil_grids'):
                    # Scale the learning rate of bilateral grids.
                    param_group['lr'] = learning_rate * config.bilgrid_lr_scale
                else:
                    param_group['lr'] = learning_rate
            ...
            if hasattr(module, 'bil_grids') and module.bil_grids is not None:
                if config.bilgrid_tv_loss_mult > 0:
                    losses['tv_bilgrids'] = train_utils.bilateral_grid_tv_loss(module, config)
                ...
                if accelerator.is_main_process:
                    ...
                    if config.rawnerf_mode:
                        ...
                        for p, x in list(exposures.items()):
                            ...
                            summary_writer.add_image(f'test_true_color_exp{p}',
